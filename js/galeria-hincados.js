...
function loadMedia(index) {
    ...
    mainViewer.innerHTML = `<img src="${item.url}" alt="Proyecto de hincado de tuberías de ER Trenchless Chile ${index + 1}" decoding="async" style="width:100%; height:100%; object-fit:contain; border-radius: 8px;">`;
...
function renderThumbnails() {
    ...
    proyectosData.forEach((item, index) => {
        ...
        thumb.innerHTML = `<img src="${item.url}" alt="Miniatura del proyecto de hincado de tuberías ${index + 1}" loading="lazy" decoding="async" style="width:100%; height:100%; object-fit:cover; border-radius: 4px;">`;
        thumb.onclick = () => {
            loadMedia(index);
            startSlideshow();
        };
